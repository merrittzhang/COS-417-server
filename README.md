...
`http://www.cs.princeton.edu/index.html`. This URL identifies that the HTTP
protocol is to be used, and that an HTML file in the root directory (`/`) of
the web server called `index.html` on the host machine `www.cs.princeton.edu`
should be fetched.
...
directly in the URL, e.g., `http://www.cs.princeton.edu:8000/index.html`.
...
`http://www.cs.princeton.edu/test.cgi`. To specify more arguments, the `?` and `&`
...
example, `http://www.cs.princeton.edu/test.cgi?x=10&y=20` can be used to send
...
`courselab01.cs.princeton.edu` and use port number 8003; copy your favorite HTML file
...
`courselab01.cs.princeton.edu:8003/favorite.html`. If you run the client and web
...
Your role in scheduling is to determine which HTTP request should be
...
careful.
Thus, security is something you should consider carefully when
creating a web server.
One thing you should always make sure to do is not leave your server running beyond testing, thus leaving open a potential
...
```sh
prompt> ./bin/pserver [-d basedir] [-p port] [-t threads] [-b buffers] [-s schedalg]
```
...
```sh
prompt> ./bin/pserver -d contents -p 8003 -t 8 -b 16 -s SFF
```

In this case, your web server will run from the base directory `contents`,
listen to port 8003, create 8 worker threads for
...
- `pserver.c`: Contains `main()` for the web server and the basic serving loop.
...
- `pclient.c`: Contains main() and the support routines for the very simple
  web client. To test your server, you may want to change this code so that it
  can send simultaneous requests to your server. By launching `pclient`
...
- `Makefile`: We also provide you with a sample Makefile that creates
  `pserver`, `pclient`, and `spin.cgi`. You can type make to create all of
...
To find information on these library routines, read the man pages.

## Testing

The starter code provided gives some basic test cases, but are certainly not exhaustive; we will run additional hidden tests on your final submission.

Make sure to thoroughly test your code and ensure that your implementation includes all requirements. Remember especially to look at possible areas where race conditions could occur.

**Your locks should be made as fine-grained as possible (i.e. only critical sections of code locked).**
We may test/check to make sure your implementation uses locks only when necessary.

Resources for debugging threads such as `Helgrind` and `ThreadSanitizer` should definitely be utilized!

## What to Turn In

Turn the `pserver.c` file in to the `server` assignment on TigerFile.
